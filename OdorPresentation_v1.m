function OdorPresentation_v1(COMPort, bee, sess)
%-------------------------------------------------------
% For example:
%   OdorPresentation_v1('COM4', 400, 2)
...
while pause_flag && (ex.tr_num < ex.n_tr)
    ...
    fprintf('%d : %d\t %d \r', ex.tr_num, tr(ex.tr_num).t0, tr(ex.tr_num).odorNum);
    
    %sentence that will be sended to Arduino
    s1 = sprintf('%s %d %d %d\r', 'params',...
                  tr(ex.tr_num).ITI_dur, tr(ex.tr_num).odorNum, tr(ex.tr_num).odorDur);
